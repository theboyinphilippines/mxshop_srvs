...
type OrderInfo struct {
	...
	OrderMount float32    //订单总金额
	PayTime    *time.Time `gorm:"type:datetime"` //用户支付时间
...
//订单商品表（一个订单有多个商品，这张表单独拿出来，不放在order表中，查询影响性能）
type OrderGoods struct {
	...
	//把商品的信息保存下来了 ， 字段冗余(goods服务中有goods表已保存)， 高并发系统中我们一般都不会遵循三范式  （做镜像记录，保存当时的价格等）(防止跨服务查询)
